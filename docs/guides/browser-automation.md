...
> [!NOTE]
> `cdp_proxy.py` 會由 docker-compose 隨 gateway 一併啟動，並讀取 data dir 內的 `scripts/host/browsers.conf`（來源是 repo root 的 [`browsers.conf`](../../browsers.conf)），**為其中每個 port 自動架好轉發**（前提：`browsers.conf` 已在掛載進容器的 data dir 內）。所以 conf 裡已列的 port 不必再手動啟動轉發站；下面的手動 `docker exec ... cdp_proxy.py <port>` 只用於臨時補一個 conf 沒列的 port。
