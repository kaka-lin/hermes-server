...
- `cdp_proxy.py` 由 docker-compose 在容器啟動時一併拉起（`python3 .../cdp_proxy.py & exec hermes gateway run`），並讀取 data dir 內的 `scripts/host/browsers.conf`（由 `hermes-run.sh` 在 `new` 與每次 `up` 時從 repo root 的 `browsers.conf` 同步進去，與 host 端 `start-browsers.sh` 讀的是同一份清單），會為 conf 裡每個 port 各開一條轉發，無需手動逐一指定。
...
本專案內建這支腳本：[`start-browsers.sh`](../../start-browsers.sh)（repo root）。Profile 與 port 對應寫在同目錄的 `browsers.conf`（純文字，每行 `port:profile_name`）。
...
```bash
cp browsers.conf.example browsers.conf
...
# 啟動所有 profile
./start-browsers.sh
./start-browsers.sh start    # 同上，顯式寫法

# 看誰 listen
./start-browsers.sh status

# 全部關掉（不用記 pkill 指令）
./start-browsers.sh stop

# 重啟
./start-browsers.sh restart
```
